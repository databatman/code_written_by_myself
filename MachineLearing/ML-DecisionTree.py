...
##################################################################################################################
'''
注：这个算法的逻辑结构并不是我自己想出来的，是我在看过集体智慧编程之后，再重写了一遍实现的。
    除了这个决策树，之后的算法：感知器、k-means、knn 都是我自己构思实现的。
'''
##################################################################################################################
...
def main():                  #以下内容为我测试决策树的代码  
    a=BuildTree(my_data)  
    print 'this is original decision tree. '  
    PrintTree(a)    #打印决策树结果
    '''会绘制决策树的图形，需要zlib库'''
    '''若包含zlib库，可把下面那句代码的注释去掉执行获得决策树图形'''
    #drawtree(a,jpeg='treeview.jpg')   
    #对决策树进行剪枝，设置最小阈值0.01，当分类后信息增益小于0.01的分类将被去掉
    prune(a,0.1)  
    print '\nthis is decision tree with prune(mingain=0.1)'
    PrintTree(a)  
    prune(a,1)
    print '\nthis is decision tree with prune(mingain=1)'
    PrintTree(a)
